...
function Renderer(ws,canvas, world, player)
{
	// local references
    this.ws = ws;
	this.canvas = canvas;
...
Renderer.prototype.render = function()
{
	...
	if(this.mouseClick)
	{
		...
		if(this.clickedNode && this.mouseClick.button == 0)
		{
			...
			{
                // since the server regulates block placement/removal
                // wait for it to send the okay before
                // placing the node
                // this.world.addNode(newNode.x, newNode.y, newNode.z, selectedType);

                //: signal the server about node placement
                this.ws.send(JSON.stringify({
                    type: MSG.TYPE.REQUEST_BLOCK_PLACE,
                    data:{
                        x: newNode.x,
                        y: newNode.y,
                        z: newNode.z,
		...
		else if(this.clickedNode && this.mouseClick.button == 2)
		{
            ...
            //: signal the server about node placement
            this.ws.send(JSON.stringify({
                type: MSG.TYPE.REQUEST_BLOCK_DESTROY,
                data:{
                    x: this.clickedNode.x,
                    y: this.clickedNode.y,
                    z: this.clickedNode.z
                }
            }));
		}
		this.clickedNode = false;
