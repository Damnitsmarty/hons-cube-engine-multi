...
function World(seed,x,z)
{
	this.chunks = {};
	this.newMap(seed,x,z);
}

World.prototype.newMap = function(seed,x,z)
{
    x = x || 0;
    z = z || 0;
...
World.prototype.setSpawn = function(x,z){
    this.spawn = {
        x: x,
        y: this.map.getAbsoluteHeight(x, z)+3,
        z: z
    };
}
// ############################################################ MAP METHODS
...
// takes care of new chunks loading
World.prototype.mapChanges = function(serverChanges){
    var wld = this;
    serverChanges.forEach(function(chunk){
        var cx = chunk.position.x;
        var cz = chunk.position.z;
        // update chunk
        wld.applyChanges(chunk.changes);
    });
}
World.prototype.applyChanges = function (changes) {
    var wld = this;
    Object.keys(changes).forEach(function(key){
        var type = changes[key];

        var currentNode = wld.getNode(...key.split('_').map((item) => parseInt(item)));
        if(currentNode)
            wld.removeNode(currentNode);
        if(typeof(type) === 'string')
            wld.addNode(...key.split('_').map((item) => parseInt(item)),type);
    });
};
// ############################################################ CHUNK METHODS
