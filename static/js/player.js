...
function Player(ws,world)
{
    // store the connection to the server
    this.ws = ws;
	this.world = world;

	// player position is limited by Number.MAX_VALUE (1.798e308) and Number.MIN_VALUE (5e-324)
    // use object.assign so the values are copied!
    this.position = Object.assign({}, this.world.spawn);
	...
	{
        if(event.code == "Enter") document.getElementById('txtMessage').focus();
...
Player.prototype.spawn = function()
{
    ...
    // map the grid immediately so the player does not fall below the surface
    this.world.mapGrid9(this.chunk.x, this.chunk.z);
    // send a request for chunk info
    this.ws.send(JSON.stringify({
        type: MSG.TYPE.REQUEST_CHUNK_INFO,
        data: {
            x: this.chunk.x,
            z: this.chunk.z
        }
...
Player.prototype.update = function()
{
    ...
    if(((Math.abs(this.delta.x) > 0) ||
        (Math.abs(this.delta.y) > 0) ||
        (Math.abs(this.delta.z) > 0)) &&
        (this.position.y > 0)){
            // signal server about Movement
            this.ws.send(JSON.stringify({
                type: MSG.TYPE.PLAYER_MOVED,
                data:{
	...
	if(cx != this.chunk.x || cz != this.chunk.z)
	{
        ...
        // instead of mapping the chunks, wait for the
        // server to send block info about them
        //this.world.mapGrid9(this.chunk.x, this.chunk.z);
        this.ws.send(JSON.stringify({
            type: MSG.TYPE.REQUEST_CHUNK_INFO,
            data: {
